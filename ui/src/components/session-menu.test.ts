...
describe("session menu", () => {
  ...
  it("dispatches favorite toggles", async () => {
    const onAction = vi.fn<(action: SessionMenuAction) => void>();
    const menu = await mountMenu({ onAction });

    menuItem(menu, "Add favorite").click();
    expect(onAction).toHaveBeenCalledWith({ kind: "toggle-favorite" });
  });

  it("shows remove favorite when already favorited", async () => {
    const menu = await mountMenu({ session: { permanentFavorite: true } });

    expect(menuItemLabels(menu)).toContain("Remove favorite");
    expect(menuItemLabels(menu)).not.toContain("Add favorite");
  });
