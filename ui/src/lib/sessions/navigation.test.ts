...
describe("resolveSessionNavigation", () => {
  ...
  it("keeps permanent favorites ahead of pins and outside the recent-session cap", () => {
    const favoriteSessions = [
      {
        key: "agent:main:favorite-b",
        kind: "direct" as const,
    ...
    const navigation = resolveSessionNavigation({
      result: sessionsResult([
        ...recentSessions,
        { key: "agent:main:pinned", kind: "direct", pinned: true, updatedAt: 500 },
        ...favoriteSessions,
      ]),
      resultAgentId: "main",
      sessionKey: "unknown",
    ...
    expect(navigation.recentSessions.map((row) => row.key)).toEqual([
      "agent:main:favorite-a",
      "agent:main:favorite-b",
      "agent:main:pinned",
