...
describe("sessions view", () => {
  ...
  it("favorites, pins, archives, and restores sessions from row actions", async () => {
    const container = document.createElement("div");
    const onPatch = vi.fn();
    render(
      renderSessions({
    ...
    container.querySelector<HTMLButtonElement>('button[title="Add favorite"]')!.click();
    ...
    expect(onPatch).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ permanentFavorite: true, favoriteOrder: expect.any(Number) }),
    );
    expect(onPatch).toHaveBeenNthCalledWith(2, "agent:main:dashboard:1", { pinned: true });
    expect(onPatch).toHaveBeenNthCalledWith(3, "agent:main:dashboard:1", { archived: true });
