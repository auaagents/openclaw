...
function sessionDetailItems(params: {
  ...
  if (typeof row.permanentFavorite === "boolean") {
    details.push({
      label: t("sessionsView.favorite"),
      value: row.permanentFavorite ? t("common.yes") : t("common.no"),
    });
  }
  return details;
}
    ...
    >
      ...
      <td class="session-actions-cell">
        <div class="session-actions">
          ...
          <button
            class="icon-btn"
            title=${row.permanentFavorite
              ? t("sessionsView.removeFavorite")
              : t("sessionsView.addFavorite")}
            aria-label=${row.permanentFavorite
              ? t("sessionsView.removeFavorite")
              : t("sessionsView.addFavorite")}
            ...
            @click=${(event: MouseEvent) => {
              ...
              props.onPatch(
                row.key,
                row.permanentFavorite === true
                  ? { permanentFavorite: false, favoriteOrder: null }
                  : { permanentFavorite: true, favoriteOrder: Date.now() },
              );
            }}
          >
            ${icons.bookmark}
